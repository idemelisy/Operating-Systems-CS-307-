...
int main(int argc, char* argv[]) {
    ...
    processor_queues = malloc(NUM_CORES * sizeof(SortedDispatcherDatabase*));

    // Initialize shared variables, call the student's function as well.
    for (int i = 0; i < NUM_CORES; i++) {
        processor_queues[i] = malloc(sizeof(SortedDispatcherDatabase));
        finished_jobs[i] = 0;
    }
    initSharedVariables();
